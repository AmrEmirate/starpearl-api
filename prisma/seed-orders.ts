...
async function main() {
  ...
  if (!buyer) throw new Error("Buyer not found. Run seed-full.ts first.");

  // Get buyer's address
  const address = await prisma.address.findFirst({
    where: { userId: buyer.id },
  });
  if (!address) throw new Error("Address not found. Run seed-full.ts first.");

  // Get seller's store
  const seller = await prisma.user.findUnique({
    where: { email: "seller@test.com" },
  });
  if (!seller) throw new Error("Seller not found. Run seed-full.ts first.");

  const store = await prisma.store.findUnique({
    where: { userId: seller.id },
  });
  if (!store) throw new Error("Store not found. Run seed-full.ts first.");
  ...
  for (const { status, count, paymentStatus } of statuses) {
    for (let i = 0; i < count; i++) {
      ...
      const serviceFee =
        subtotal < 50000 ? 0 : Math.ceil(subtotal / 100000) * 1000;
