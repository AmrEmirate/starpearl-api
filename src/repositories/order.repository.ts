...
export class OrderRepository {
  ...
  async createOrderFromCart(input: CreateOrderInput): Promise<Order> {
    const { 
      userId, cart, addressId, shippingCost, 
      serviceFee, totalPrice, subtotal, // Ambil subtotal dari input
      logisticsOption, paymentMethod 
    } = input;
      ...
      const order = await prisma.$transaction(async (tx) => {
        ...
        for (const item of cart.items) {
          ...
          if (availableStock === undefined || availableStock < item.quantity) {
            throw new AppError(`Not enough stock for product: ${nameMap.get(item.productId)}. Available: ${availableStock || 0}`, 400);
          }
        }
